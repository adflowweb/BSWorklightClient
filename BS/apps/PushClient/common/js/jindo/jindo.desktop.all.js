...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
    ...
    if (/(\$\w+)#(\w+)?/.test(sFuncName)) {
        sURL = 'http://jindo.dev.naver.com/docs/jindo/archive/Jindo2-2.9.1/desktop/ko/classes/' + encodeURIComponent(RegExp.$1) + '.html' + "#method_"+RegExp.$2;
    }
...
jindo.$Ajax = function (url, option) {
    ...
    // switch (oArgs.oOption.type) {
        // case "put":
        // case "delete":
        // case "get":
        // case "post":
            // oArgs.oOption.type = "xhr";
    // }
...
xhr인 경우 IE에서는 GET으로 보낼 때 브라우져에서 자체 cache하여 cache을 안되게 수정.
	...
	if (sUpType=="XHR"||sUpType=="IFRAME"||(sUpType=="FLASH"&&opt.sendheader)) {
		if(!this._headers["Content-Type"]){
			req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=utf-8");
		}
		req.setRequestHeader("charset", "utf-8");
		if(!this._headers["X-Requested-With"]){
        ...
        revisionEvent : function(sType, sEvent, realEvent){
            if (document.addEventListener !== undefined) {
                this.revisionEvent = function(sType, sEvent, realEvent){
                    //IE같은 경우 대소문자를 구분하여 prefIx가 ms이면 그냥 반환.
...
//     
    ...
    // }
    // var delay = jindo._p_._j_ag.indexOf("Android") > -1 ? 500 : 100;
    // var type;
    // if(hasOrientation){
